                         ...
                         const core::GameWorld& world) const
{
  ...
  if (m_screen == Screen::Playing) {
    ...
    renderer.drawUiRect(637.0f * sx, 12.0f * sy,
                        96.0f * sx * fireCharge, 12.0f * sy, kFire);
    renderer.drawUiText("THRUST", 535.0f * sx, 44.0f * sy, 2.0f * sy, kText);
    ...
    renderer.drawUiText("SHIELD", 535.0f * sx, 75.0f * sy, 2.0f * sy, kText);
    const float shieldCharge =
        std::clamp(world.shieldChargeFraction(), 0.0f, 1.0f);
    renderer.drawUiRect(635.0f * sx, 72.0f * sy, 100.0f * sx, 16.0f * sy,
                        kShieldTrack);
